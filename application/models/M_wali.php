...
class M_wali extends CI_Model{
    ...
    public function getData($type, $page, $sortname, $sortvalue, $wherename, $wherevalue)
    {
		...
		$query_totaldata = $this->db->get('s_wali');
		...
		$query = $this->db->get('s_wali', $this->max_per_page, $page);
    ...
    public function addWali($request) 
    {
        // menambahkan data Wali pada request berdasarkan data json
        // data $request akan dimasukan pada fungsi addWali ini melalui controller.
        $query = $this->db->insert('s_wali', $request);
        if($query) {
            // jika query berhasil maka httpcode yang diberikan adalah 200(success)
            $this->response['status_code'] = 200;
            $this->response['status_message'] = "data Wali berhasil disimpan";
    ...
    public function editWali($kd_wali,$request)
    {
        // data $request dan $kd_wali akan dimasukan pada fungsi editWali ini melalui controller.
        $this->db->where('kd_wali', $kd_wali);
        $query = $this->db->update('s_wali', $request);
        if($query) {
            // jika query berhasil maka httpcode yang diberikan adalah 200(success)
            $this->response['status_code'] = 200;
            $this->response['status_message'] = "data Wali berhasil diubah";
            $this->response['data'] = $request;
        }else{
            // jika query gagal atau error maka akan menampilkan httpcode 500(internal server error)
            $this->response['status_code'] =  500;
    ...
    public function deleteWali($kd_wali)
    {
        // data $request dan $kd_wali akan dimasukan pada fungsi deleteWali ini melalui controller.
        $this->db->where('kd_wali', $kd_wali);
        $query = $this->db->delete('s_wali');
        if($query) {
            // jika query berhasil maka httpcode yang diberikan adalah 200(success)
            $this->response['status_code'] = 200;
            $this->response['status_message'] = "data Wali berhasil dihapus";
            $this->response['data'] = $request;
        }else{
            // jika query gagal atau error maka akan menampilkan httpcode 500(internal server error)
            $this->response['status_code'] =  500;
